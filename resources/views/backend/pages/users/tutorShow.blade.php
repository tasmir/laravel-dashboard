    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.users.edit', $user->id) }}" class="inline-flex items-center justify-center px-4 py-2 border border-slate-300 shadow-sm text-sm font-medium rounded-lg text-slate-700 bg-white hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue">
                ...
                <form action="{{ route('admin.users.update', $user->id) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                    @csrf
                    @method('PUT')
                    <button type="submit" class="inline-flex items-center justify-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white bg-blue hover:bg-blue focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue">
                ...
                <div class="relative flex items-end -mt-12 mb-4">
                    <div class="relative rounded-xl border-4 border-white shadow-md bg-white p-1">
                        <img class="h-24 w-24 rounded-lg object-cover" 
                             src="{{ $user?->media?->src ? asset($user->media->src) : 'https://ui-avatars.com/api/?name='.urlencode($user->name).'&color=7F9CF5&background=EBF4FF' }}" 
                             alt="{{ $user->name }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            <p class="text-sm font-medium text-slate-500">Primary Address</p>
                            <p class="mt-1 text-sm text-slate-900">{{ $user->address_line }}</p>
                        </div>
                    ...
                    @else
                        <p class="text-sm text-slate-400 italic">No address information provided.</p>
                    @endif
                </div>
                    ...
                    <dl class="space-y-4">
                        <div class="flex justify-between items-center pb-3 border-b border-slate-100 last:border-0 last:pb-0">
                            <dt class="text-sm text-slate-500">Email Verified</dt>
                             <dd class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $user->email_verified_at ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                {{ $user->email_verified_at ? 'Verified' : 'Pending' }}
                            </dd>
                        </div>
                        <div class="flex justify-between items-center pb-3 border-b border-slate-100 last:border-0 last:pb-0">
                            <dt class="text-sm text-slate-500">Mobile Verified</dt>
                             <dd class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $user->mobile_verified_at ?? false ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                {{ $user->mobile_verified_at ?? false ? 'Verified' : 'Pending' }}
                            </dd>
                        </div>
                         <div class="flex justify-between items-center pb-3 border-b border-slate-100 last:border-0 last:pb-0">
                            <dt class="text-sm text-slate-500">Tutor Verified</dt>
                             <dd class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $user->tutorProfile && $user->tutorProfile->approved_at ? 'bg-blue/20 text-blue' : 'bg-slate-100 text-slate-800' }}">
                                {{ $user->tutorProfile && $user->tutorProfile->approved_at ? 'Approved' : 'Unapproved' }}
                            </dd>
                        </div>
                    </dl>
                </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    @else
                        <p class="text-sm text-slate-400 italic">No biography provided by this user.</p>
                    @endif
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    @else
                        <div class="text-center py-6 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                            <p class="text-sm text-slate-500">No academic records found.</p>
                        </div>
                    @endif
                </div>
                
                 {{-- Tutoring Subjects & Preferences --}}
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div class="grid md:grid-cols-2 gap-8">
                        <div>
                             ...
                             <div class="flex flex-wrap gap-2">
                                @if($user->tutorProfile && $user->tutorProfile->preferencesSubject->count() > 0)
                                    @foreach($user->tutorProfile->preferencesSubject as $pref)
                                    ...
                                    @endforeach
                                @else
                                     <span class="text-sm text-slate-400 italic">No preferences set</span>
                                @endif
                             </div>
                        </div>
                         ...
                         <div>
                             ...
                             <div class="flex flex-wrap gap-2">
                                @if($user->tutorProfile && $user->tutorProfile->preferencesMedium->count() > 0)
                                    @foreach($user->tutorProfile->preferencesMedium as $pref)
                         ...
                         <div class="md:col-span-2">
                             ...
                             <div class="flex flex-wrap gap-2">
                                @if($user->tutorProfile && $user->tutorProfile->preferencesLevel->count() > 0)
                                    @foreach($user->tutorProfile->preferencesLevel as $pref)
                ...
                {{-- Recent Login / Technical Info (Optional, fitting for admin) --}}
